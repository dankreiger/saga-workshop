...
import {
  dogSaga,
  watchFetchDogRequest,
  fetchDogRequestAsync
} from "./sagas"
import {
  DogActionTypes
} from './constants';
...
describe('all dog sagas', () => {
  ...
  describe('watchFetchDogRequest', () => {
    ...
    it('watches for the latest action of type "FETCH_DOG_REQUEST"', () => {
      expect(gen.next().value).toEqual(takeLatest(DogActionTypes.FETCH_DOG_REQUEST, fetchDogRequestAsync))
    })
  })
  ...
  describe('fetchDogRequestAsync', () => {
    ...
    it('calls the dog api, receives a successful response, and then dispatches fetchDogSuccess', () => {
      ...
      expect(gen.next({message: 'some/image.png'}).value).toEqual(put(fetchDogSuccess('some/image.png')));
    })
